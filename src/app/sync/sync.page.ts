...
export class SyncPage implements OnInit {
  ...
  showMedia(item) { 
    if(item.doc_type == 155) {  
      var videopath = this.webview.convertFileSrc(this.file.externalRootDirectory + 'icollect_survey/documents/' + item.filename);
      this.openMeia(videopath);

    } else
    if(item.doc_type == 157) {  
      var audioPath = this.file.externalRootDirectory + 'icollect_survey/documents/' + item.filename;
      ...
      if(item.cloud_path!=null) {
        ...
        correctPath = this.file.externalRootDirectory + 'icollect_survey/documents/' + item.filename; 
      }
  ...
  sync() { 
    if (this.network == true) {
      ...
      this.data.forEach(value => { 
        let filepath = this.file.externalRootDirectory + 'icollect_survey/documents/';
