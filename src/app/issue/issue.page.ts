...
export class IssuePage implements OnInit {
  ...
  sendData(created_date, survey_response_id) {
    if (this.network == true) {
      ...
      var myData = JSON.stringify({
        ...
        ACTION_STATUS: this.status_name,
  ...
  captureAudio() {
    ...
    this.mediaCapture.captureAudio(options).then((res: MediaFile[]) => { 
      ...
      this.presentPrompt(157, fromDirectory+'/', fileName, this.survey_response_id, null);
  ...
  captureVideo() {
    ...
    this.mediaCapture.captureVideo(options).then((res: MediaFile[]) => { 
      ...
      this.presentPrompt(155, fromDirectory+'/', fileName, this.survey_response_id, null);
  ...
  moveFile(description, correctPath, currentName, survey_unique_id, doc_type, source) {
    ...
    if (doc_type == 155) {
      ...
      this.file.moveFile(correctPath, currentName, newPath, newFileName).then(() => { 
        this.db.saveDocData(this.user.id_contact, survey_unique_id, newFileName, description, doc_type);
      });
