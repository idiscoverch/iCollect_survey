...
export class SurveyPage implements OnInit {
  ...
  moveFile(description, correctPath, currentName, survey_response_id, doc_type) {
    var m = new Date();
    let created_date = m.getUTCFullYear() + "-" + ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" + ("0" + m.getUTCDate()).slice(-2) + " " + ("0" + m.getUTCHours()).slice(-2) + "-" + ("0" + m.getUTCMinutes()).slice(-2) + "-" + ("0" + m.getUTCSeconds()).slice(-2);
    let newPath = this.file.externalRootDirectory + 'icollect_survey/documents/';
    ...
    if (doc_type == 155) {
      ...
      this.file.moveFile(correctPath, currentName, newPath, newFileName).then(_ => {
        this.db.saveDocData(this.user.id_contact, this.survey_response_id, newFileName, description, doc_type);
      });
    ...
    if(doc_type == 157) {
      ...
      this.file.moveFile(correctPath, currentName, newPath, newFileName).then(_ => {
        this.db.saveDocData(this.user.id_contact, survey_response_id, newFileName, description, doc_type);
      });
