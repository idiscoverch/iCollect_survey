...
export class DatabaseService {
  ...
  createSurveyDir() {
    this.file.checkDir(this.file.externalRootDirectory, 'icollect_survey').then(response => {
      console.log(response);
    }).catch(err => { 
      console.log(err);
      this.file.createDir(this.file.externalRootDirectory, 'icollect_survey', true).then(response => { 
  ...
  createDocumentsDir() {
    this.file.checkDir(this.file.externalRootDirectory, 'icollect_survey/documents').then(response => {
      console.log(response);
    }).catch(err => {
      console.log(err);
      this.file.createDir(this.file.externalRootDirectory, 'icollect_survey/documents', true).then(response => {
        console.log('Directory create' + response);
      }).catch(err2 => { console.log('Directory no create' + JSON.stringify(err2)); });
    });
  }
  ...
  restFetchAgentDeploymentLenth(id_country, admin, id_project) {
  ...
  restFetchScopesLenth(id_country, id_project) {
  ...
  loadScopespartner(id_country, id_project, id_scope) {
  ...
  saveDocData(id_contact, survey_unique_id, filename, description, doc_type) {
    var m = new Date();
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.getNewId().then(val => {
        let data = [val.new_id, id_contact, survey_unique_id, date, doc_type, filename, resp.coords.latitude, resp.coords.longitude, resp.coords.accuracy, resp.coords.heading, resp.coords.altitudeAccuracy, description, 0];

        return this.database.executeSql('INSERT INTO documnts (id_doc, agent_id, survey_unique_id, doc_date, doc_type, filename, coordx, coordy, accuracy, heading, altitude, description, sync) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', data).then(_ => {
  ...
  loadDocumentsSync() {
    return this.database.executeSql('SELECT id_doc, doc_type, cloud_path, agent_id, survey_unique_id, filename, description, doc_date, sync, coordx, coordy, accuracy, heading, altitude FROM documnts WHERE sync != 1 AND cloud_path IS NULL', []).then(data => {
          ...
          if (data.rows.item(i).cloud_path != null) {
            media_path = this.pathForImage(data.rows.item(i).cloud_path);
              ...
              if(data.rows.item(i).doc_type == 155){
                ...
                let filePath = this.file.externalRootDirectory + 'icollect_survey/documents/' + data.rows.item(i).filename;
  ...
  loadSurveyResults(id_contact) {
    return this.database.executeSql('SELECT COUNT(*) AS total, regvalues.cvalue, regvalues.cvalueind, regvalues.cvaluefr, sv_results.created_date, towns.code_town, scope_partner.contact_code FROM sv_results, regvalues, towns, scope_partner WHERE regvalues.id_regvalue = sv_results.scope_id AND towns.gid_town = sv_results.town_id AND scope_partner.id_contact = sv_results.partner_id AND sv_results.id_agent = ? AND sv_results.survey = 1 GROUP BY survey_unique_id', [id_contact]).then(data => {
      let answerList: any[] = [];
  ...
  loadIssueResults(id_contact) {
    return this.database.executeSql('SELECT COUNT(*) AS total, regvalues.cvalue, regvalues.cvalueind, regvalues.cvaluefr, sv_results.created_date, towns.code_town, scope_partner.contact_code FROM sv_results, regvalues, towns, scope_partner WHERE regvalues.id_regvalue = sv_results.scope_id AND towns.gid_town = sv_results.town_id AND scope_partner.id_contact = sv_results.partner_id AND sv_results.id_agent = ? AND sv_results.issue = 1 GROUP BY survey_unique_id', [id_contact]).then(data => {
      let answerList: any[] = [];
  ...
  answersNotSynced() {
    return this.database.executeSql('SELECT *, sv_results.created_date FROM sv_results, regvalues WHERE regvalues.id_regvalue = sv_results.scope_id AND sync_to_server = 0', []).then(data => {
      let answerList: any[] = [];
      ...
      if (data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          ...
          answerList.push({
            ...
            id_scope: data.rows.item(i).id_scope,
            id_partner: data.rows.item(i).id_partner,
            ...
            datte_answer: data.rows.item(i).datte_answer,
