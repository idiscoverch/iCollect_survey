...
export class DownloadPage implements OnInit {
    ...
    this.activatedRoute.paramMap.subscribe(param => {
      ...
      this.db.lastLogedUser().then(usr => {
        ...
        if(this.type == 'answers') {   
          this.login = false;
          this.restFetchAnswers(this.id_project);
  ...
  restFetchTowns(id_country, id_project) {
      ...
      this.db.deleteTowns().then(() => {
        this.http.get(link, {}, {}).then(data => {
          ...
          if (lenth == 0) { 
            ...
            this.loadTowns();
            this.towns_spinner = false;
            this.restFetchAnswers(id_project);
  ...
  restFetchAnswers(id_project) {
    ...
    this.db.restFetchAnswersLenth().then(total_rows => {
      ...
      let link = 'https://survey.icertification.ch/ords/m/v_sv_answers_def/?limit=10000';
        ...
        this.http.get(link, {}, {}).then(data => {
          ...
          if (lenth == 0) { 
            ...
            if(this.type == 'answers'){
              ...
              this.restFetchScope(id_project);
            }
  ...
  restFetchScope(id_project) {
    this.scope_spinner = true;
    this.scope_progress = 0;

    this.db.restFetchScopesLenth(this.user.id_country, id_project).then(total_rows => {
      this.scope_total_row = total_rows;
      this.scope_total = total_rows;
      
      let link = 'https://survey.icertification.ch/ords/m/v_scope_partner/?limit=1000&q={"id_country":"'+this.user.id_country+'","id_project":"'+id_project+'"}';
